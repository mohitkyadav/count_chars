            ...
            <v-toolbar color="blue darken-3">
              <v-toolbar-items>
                <v-layout align-center="true" justify-center="true">
                  <v-chip
                    class="elevation-16 uk-animation-slide-left-small"
                    color="secondary"
                    small
                    text-color="white"
                    v-for="i in fitlerZeroCounts()"
                    :key="i.type"
                  >
                ...
                <v-flex md12 offset-xs0>
                  <v-card class="card--flex-toolbar elevation-16" height="fit-content">
                    <v-toolbar color="darken-3" card prominent>
                      ...
                      >Paste or start typing text in the box below</v-toolbar-title>
                      <v-spacer></v-spacer>
                      <v-btn icon>
...
<script>
/* eslint-disable*/ // My IDE went mad
export default {
  name: "Landing",
  data() {
    return {
...
function isVowel(c) {
  let vowASCII = [65, 97, 69, 101, 73, 105, 79, 111, 85, 117];
  return vowASCII.includes(c);
}
function isLowerCap(c) {
  return c >= 97 && c <= 122;
}
function isNumber(c) {
  return c >= 48 && c <= 57;
